...
app.post("/temperatures/upload", (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const temperatureFileString = req.files.temperatureFile.data.toString(
        "utf8"
      );
      ...
      logger.debug(
        `Uploaded file "${req.files.temperatureFile.name}" was parsed successfully.`
      );
    } catch (ex) {
